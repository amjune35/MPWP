---
title: "Genera V3 Hill Rare 03_17_25"
author: "Alicia June"
date: "2025-03-17"
...
This document is V3 of conducting rarefaction using Hill Numbers. The data is being used is genera ONLY data (03-17-2025). V3 uses 5 samples for each member. It excludes the following samples: 
...
```{r Importing data}
genRAW<- read_csv("gen031325.csv", col_names = TRUE)
genRAW
```
...
write_csv( SM_samples_observed, "SM_gen_ObsV3.csv")
SM_samples_observed
```
...
write_csv(SM_rare_data, "SM_gen_rareV3.csv")
head(SM_rare_data)
```
...
write_csv( RM_samples_observed, "RM_gen_ObsV3.csv")
RM_samples_observed
```
...
write_csv(RM_rare_data, "RM_gen_rareV3.csv")
head(RM_rare_data)
...
write_csv( MH_samples_observed, "MH_gen_ObsV3.csv")
...
write_csv(MH_rare_data, "MH_gen_rareV3.csv")
head(MH_rare_data)
```
