---
title: "Species Hill Numbers"
author: "Alicia M. June"
output: html_document
---
# Introduction

This document contains code for calculating the observed Hill numbers (Fig X, X, X) and the diversity curves (Fig X, X, X). The data is being used is species ONLY data. 5 samples were used for each member.
...
```{r Importing data}
spONLYraw<- read_csv("spRAW.csv", col_names = TRUE)
spONLYraw
```
...
write_csv( SM_samples_observed, "SM_spONLY_Obs.csv")
SM_samples_observed
```
...
write_csv(SM_rare_data, "SM_spONLY_rare.csv")
head(SM_rare_data)
```
...
write_csv( RM_samples_observed, "RM_spONLY_Obs.csv")
RM_samples_observed
```
...
write_csv(RM_rare_data, "RM_spONLY_rare.csv")
head(RM_rare_data)
...
write_csv( MH_samples_observed, "MH_spONLY_Obs.csv")
...
write_csv(MH_rare_data, "MH_spONLY_rare.csv")
head(MH_rare_data)
```
