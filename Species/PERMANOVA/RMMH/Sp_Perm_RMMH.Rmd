...
This document contains code to conduct PERMANOVA comparing the Rushmere and Moore House members of the Yorktown Formation. The distance matrix is calculated using species ONLY proportional abundance data. 5 samples were used for each member.

The goals of this code are the following:

* Run PERMANOVA on species ONLY data, compare Rushmere and Moore House Members
...
### Just Numeric Data
```{r Just nums}
spONLY_RMMH<- spONLY_raw[spONLY_raw$Member != "SM",]
spONLY_RMMH

#just numeric data
spONLY_RMMH_num<-spONLY_RMMH[,3:198] 
spONLY_RMMH_num
```
...
RMMH_perm_data<- (100*(spONLY_RMMH_num/rowSums(spONLY_RMMH_num)))
RMMH_perm_data
...
rownames(perm_distRMMH_df)<- spONLY_RMMH$`Sample ID`
colnames(perm_distRMMH_df)<- spONLY_RMMH$`Sample ID`

write_csv(perm_distRMMH_df, "sp_dist_RMMH.csv")
perm_distRMMH_df
```
...
dispersion<-betadisper(perm_distRMMH, group=spONLY_RMMH$Member,type = "centroid")
